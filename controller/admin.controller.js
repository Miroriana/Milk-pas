...
const addVeterian = async (req, res, next) => {
  ...
  try {
    var veterianExists = await VeterinaryModel.findOne({ email: req.body.email });
    if (veterianExists){
      return res
        .status(200)
  ...
  const removeVeterinary = async (req, res, next) => {
    const {email, ...rest} = req.body;
    try {
        var deletedVeterinary = await VeterinaryModel.findByIdAndDelete(req.query.id);
    ...
    const updateVeterinary = async(req,res,next) =>{
        ...
        try {
            var updatedVeterinary = await VeterinaryModel.findByIdAndUpdate({_id:req.query.id}, req.body);
            res.status(200).json({
    ...
    module.exports = { addVeterian, removeVeterinary,findVeterinaryById,listOfVeterinary,updateVeterinary};
